...
fn main() -> Result<()> {
    ...
    let s_a = [0, 2, 3]; // T1, T3, T4
    let s_b = [1, 4, 5]; // // T2, T5, T6


    ////
    // remove taxa that are not in both samples A and b. 
    ...
    // This is not preferred since it requires a lot of memory for large trees. 
    ...
    // Computation without matrix product, it is preferred, it consumes less memory for large trees
